...
# ⚠️ 注意：st.set_page_config 必須放在所有 Streamlit 指令最前面
st.set_page_config(page_title="ULD 自動打板與配載系統", layout="wide")


# 1. 解析 Excel 頂部 ULD 航司配額與容量表 (動態邊界)
...
def generate_uld_plan_bounded(cargo_df, uld_slots):
  ...
  for pool in ["5J", "CX", "CRK", "MAIN"]:
    pool_items = items[items["Target_Pool"] == pool]
    if pool_items.empty:
      continue
    ...
    available_slots = (
        ...
        if pool in pool_ulds
        else pool_ulds["CX"] + pool_ulds["RH"]
    )
  ...
  if not overflow_items.empty:
    for idx, row in overflow_items.iterrows():
      plan_rows.append({
          ...
          "容量利用率": "N/A",
          "裝載貨物組合": (
              f"⚠️ 航司配額不足: {row['Customer']} ({int(row['Vol_Rem'])} Vol)"
          ),
      })
